...
질문: {question}
학생 답변: """{answer}"""
(서재 책이면) 책 본문 — 질문과 판정의 근거는 여기서 찾아라: """{book_text ≤ 12,000자}"""

세 가지를 판정하라.

1. logic_consistency — 답변이 독후감의 주장과 어긋나지 않는가. pass / weak / fail
   책 본문이 주어지면 답변이 본문과 어긋나는지도 여기서 본다. 기준은 "읽었다면 할 수 없는 답"이다 — 본문에 전혀 없는 인물·사건을 근거로 삼거나, 본문의 낱말을 다른 뜻으로 쓴 답변(예: 총 쏘는 사냥꾼 '포수'를 야구 포수로)은 독후감과 말이 맞아도 fail 이다.
   fail 은 답변이 통째로 책과 무관할 때만이다. 본문에 실제로 있는 장면·인물을 하나라도 맞게 짚었으면, 나머지가 틀리거나 지어낸 대목이 섞여 있어도 fail 이 아니라 weak 이다 — 읽고도 기억이 흐려질 수 있고, 여기서 보는 것은 읽었는지이지 암기했는지가 아니다. 장면의 순서·정확한 표현·누가 먼저 했는지가 틀린 정도는 pass 다.
...
**책 본문 (#120, 2026-09-17).** 채점 입력에 제목만 들어가서, 모델이 줄거리를 모르는 서재 책(1920년대 단편)에서는 책과 무관한 답도 논리만 맞으면 통과했다 — 「눈 어두운 포수」의 사냥꾼 포수를 야구 포수로 답해도 pass. 서재 책은 `book_contents` 전문이 있으므로(29권 중앙값 4,400자·최대 26,600자) 12,000자까지 잘라 채점에만 넘긴다 (`verification/server/context.ts` → `reader/server/text.ts`). 저작권 있는 책은 종전대로 제목만.

- **질문 생성에는 넣지 않는다.** 넣어 봤더니 본문 장면을 보기로 나열하는 양자택일 질문("밤에 움직이던 때야, 아니면 벽 더듬던 때야?")이 나와 힌트가 됐다. 질문은 독후감만 보고 만들고, 답이 책과 맞는지는 채점이 확인한다.
- **fail 은 통째로 무관할 때만.** 처음 규칙("본문에 없는 장면을 근거로 삼으면 fail")은 읽은 아이의 흐린 기억("포수가 헤맸다" — 실제로는 자라·올빼미가 도움)까지 떨어뜨렸다 (3회 중 2회). "장면을 하나라도 맞게 짚었으면 weak 까지"로 바꾸자 3/3 통과. moderate 임계값에서 weak 은 통과다.
- 검증 스크립트: `npm run ai:book-text` — 야구 포수 답(B)이 fail, 읽은 답(C)이 pass 인지 실호출로 본다.
- 비용: 본문 12k 자 ≈ 8~10k 토큰 × 채점 1회. Sonnet 5 기준 검증 1회 $0.03 안팎.
