...
## 2c. 레벨테스트 — **제출 범위 밖** (2026-09-18 결정)

온보딩에서 짧은 지문으로 읽기 수준을 진단해 `grade_level` 을 정하는 기능이 요청됐다.
**이 스펙에는 들어오지 않는다.** 설계와 보류 근거는 `docs/plan-ko.md` §14-2 에 있다.

여기 적어 두는 이유는 하나다 — 이름이 비슷한 세 가지와 섞이지 않게 하려는 것이다.
...
| **레벨테스트** | 지문 + 문항으로 읽기 수준을 **측정**해 권장 학년을 낸다 | **제출 범위 밖** |

`grade_level` 을 자동으로 정하는 코드를 제출 전에 넣지 않는다. 그 값은 지금도 앞으로도
학생이 고른 값이고, 레벨테스트가 붙으면 **권장값을 제시할 뿐 덮어쓰지 않는다** (학생이 받아들일지 고른다).
