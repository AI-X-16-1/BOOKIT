...
function Cover({ book, size }: { book: Book; size: string }) {
  if (book.cover_url) {
    ...
    return <img src={book.cover_url} alt="" className={`${size} flex-none rounded-[10px] object-cover`} />;
  }
  return <div className={`${size} flex-none rounded-[10px] ${coverClass(book)}`} />;
}
...
export function HomeScreen() {
  ...
  const [recommend, setRecommend] = useState<BookRecommendResponse | null>(null);
  const [points, setPoints] = useState<number | null>(null);
  const [streak, setStreak] = useState<number | null>(null);
  const [rank, setRank] = useState<number | null>(null);

  useEffect(() => {
    apiGet<BookRecommendResponse>("/api/books/recommend").then(setRecommend).catch(() => {});
    apiGet<PointsResponse>("/api/points").then((r) => setPoints(r.balance)).catch(() => {});
    apiGet<GrowthResponse>("/api/growth").then((r) => setStreak(r.streak.current_days)).catch(() => {});
    apiGet<ClassRankingResponse>("/api/ranking/class").then((r) => setRank(r.my_class.rank)).catch(() => {});
  }, []);
      ...
      <div className="relative">
        ...
        {showResults && (
          <ul className="absolute inset-x-0 z-10 mt-2 overflow-hidden rounded-xl bg-card shadow-card">
            {hits.map((b) => (
              <li key={b.id} className="border-b border-border-soft last:border-b-0">
                <Link href={`/write?book=${b.id}`} className="flex min-h-12 items-center gap-3 px-4 py-3">
                  <Cover book={b} size="h-8 w-8" />
                  <span className="flex-1 truncate text-[15px] font-bold text-ink">{b.title}</span>
            ...
            {hits.length === 0 && (
              <li className="px-4 py-3 text-[14px] text-muted">
                {searching ? "찾는 중…" : "그 책은 못 찾았어. 다른 제목으로 찾아볼까?"}
              </li>
            )}
          </ul>
        )}
      ...
      <Link href="/write" className="block">
        <Card raised className="flex items-center gap-3">
          ...
          <div className="min-w-0 flex-1">
            <div className="truncate text-[15px] font-bold text-ink">쓰던 독후감</div>
      ...
      <div>
        ...
        <div className="mt-3 flex flex-col gap-2.5">
          {(recommend?.books ?? []).slice(0, 5).map((b) => (
            <Link key={b.id} href={`/write?book=${b.id}`} className="block">
              <Card className="flex items-center gap-3">
                ...
                <div className="min-w-0 flex-1">
                  <div className="truncate text-[15px] font-bold text-ink">{b.title}</div>
          ...
          {recommend && recommend.books.length === 0 && (
            <p className="text-[14px] text-muted">아직 추천할 책이 없어. 위에서 찾아볼까?</p>
          )}
        </div>
      </div>
    </div>
