...
export const LEGAL = {
   ...
   * 9/19 낱말 퀴즈(AI #8)로 국외 이전의 목적·시점이 하나 늘었다 — 보내는 항목은 그대로다.
   * 개인정보보호법 §30 — 처리방침을 바꾸면 바뀐 내용을 공개해야 한다
   */
  revisedDate: "2026-09-19",
  /** 예선 심사 종료 · 본선 데모데이 */
...
function Privacy() {
  return (
    <>
      ...
      <Table
        ...
        rows={[
          [
            ...
            "AI 빈틈 분석·질문 생성·채점, 체크포인트 문항 생성·판정, 읽는 도중 낱말 퀴즈 생성",
            "학생이 독후감을 제출·답변할 때, 책잇 서재에서 한 장을 다 읽었을 때와 체크포인트에 답할 때, 서재에서 책의 25·50·75% 쪽을 지날 때 API 전송",
          ...
          [
            "Google LLC (브라우저 음성 인식)",
            "미국",
            "소리 내어 읽을 때의 목소리",
            "책잇 서재의 소리 내어 읽기 — 읽은 곳까지 글자 색을 바꾼다",
            "학생이 🎤 버튼을 누른 동안만. 크롬 등 브라우저가 직접 전송하며 책잇 서버를 거치지 않는다",
            "책잇은 목소리와 인식 결과를 저장하지 않는다. 구글 쪽 처리는 Google 개인정보처리방침을 따른다",
          ],
        ]}
      />
      <P>
      ...
      <H2>9. 시행일</H2>
      <P>
        {LEGAL.effectiveDate} (개정 {LEGAL.revisedDate} — 체크포인트·캐릭터·읽기 기록과 소리 내어
        읽기, 낱말 퀴즈 추가에 따라 1·2·4항의 처리 항목과 국외 이전 항목을 늘렸습니다)
      </P>
    </>
  );
}
