...
**지문은 아이 화면에 그대로 떠 있다.** 외워서 답하는 것이 아니라 보면서 답한다.

세 문항은 난이도가 **한 칸씩** 올라가야 한다. 순서대로:

1. **아주 쉬움** — 지문에서 **찾으면 바로 보이는** 것 하나. 한 낱말이나 짧은 한 마디로 답해도 된다.
   "누가 무엇을 했어?" · "어디에서 있었던 일이야?" · "무엇을 보고 그랬대?" 정도.
   **초1 이 읽고 손가락으로 짚을 수 있어야 한다.** 여기서 막히면 진단이 시작도 못 한다.
2. **보통** — 왜 그랬는지, 어느 대목을 보고 그렇게 생각했는지. 한두 문장이면 된다.
3. **어려움** — 흩어진 대목을 묶어야 답이 되는 것. 말하지 않은 마음, 분위기가 바뀐 지점.

1번은 **거의 모든 아이가 답할 수 있게** 만들어라. 이 진단이 찾는 것은 "무엇을 못 하나" 가
아니라 "어디까지 되나" 이고, 그건 **되는 칸에서 시작해 한 칸씩 올려야** 보인다.
셋 다 어려우면 아무것도 알 수 없고, 아이는 첫 줄에서 그만둔다.

학년을 알려 주면 거기에 맞춰라 — **초1~3 이면 1번을 더 낮추고 문장을 더 짧게 한다.**
중학생이면 1번도 한 문장으로 답할 만한 크기여도 된다. 셋 사이의 간격은 학년과 상관없이 고르게.

규칙:
- 외워야 답하는 것은 묻지 마라. 다만 **지문에 적혀 있어 찾아서 답하는 것은 괜찮다** —
  지문이 눈앞에 있으므로 그건 기억이 아니라 읽기다. 1번이 바로 그것이다.
- 보기를 주지 마라. 고르게 하면 읽지 않고도 맞힌다.
- 지문에 없는 것을 묻지 마라.
- 반말로 한 문장씩. 1번은 짧게, 3번도 아이가 두 문장 안에 답할 크기여야 한다.
- 시험처럼 들리지 않게. "맞혀 봐" 가 아니라 "어떻게 봤어" 를 묻는다.`;
...
문항은 **한 칸씩 어려워진다.** 어디까지 되는지가 곧 답이다:
- 1번 (아주 쉬움) — 지문에서 찾아 짚는가
- 2번 (보통) — 인물의 행동에 이유를 붙이는가, 근거를 지문에서 가져오는가
- 3번 (어려움) — 흩어진 대목을 묶어 말하는가

**1번만 되고 2·3번이 안 되는 것은 "못한다" 가 아니라 "여기까지" 다.** 그 아이에게는
지금 학년이 맞거나 한 단계 아래가 맞다는 뜻이지, 실패가 아니다. 1·2번이 되고 3번이
안 되면 지금 학년이 맞다. 셋 다 되면 한 단계 위를 봐도 된다.
...
/* ── AI #8 낱말 퀴즈 (2026-09-19) ──────────────────── */

/**
 * 서재에서 읽는 **도중**에 뜨는 미니게임 (기획 §4-1 의 "어휘 문제 — 책에 나온 단어 뜻 맞히기").
 * 체크포인트(AI #6)가 장 끝에서 "읽었는가" 를 묻는다면, 이건 방금 읽은 대목의 낱말 하나를
 * 보기 셋 중에서 고르는 가벼운 놀이다. 점수·책갈피·버프는 없다 (spec §2b 불변).
 *
 * 보기를 주는 이유: AI #6 이 보기를 금지한 것은 "읽었는지" 를 묻기 때문이다. 여기는 뜻을
 * 맞히는 놀이라 읽지 않고 맞혀도 잃을 것이 없고, 탭 한 번으로 끝나야 읽는 흐름을 안 끊는다.
 */
export const WORD_QUIZ_SYSTEM = `너는 아이가 책을 읽는 도중에 잠깐 쉬어 가는 낱말 퀴즈 한 문제를 만든다.

주어진 것은 아이가 방금 읽은 대목이다.

낱말 고르기:
- 그 대목에 실제로 나온 낱말 하나. word 에는 대목에 적힌 꼴을 **한 글자도 바꾸지 말고** 그대로 적어라
  (조사·어미가 붙어 있으면 붙은 채로).
- 그 학년 아이가 처음 보거나 헷갈릴 만하지만, **오늘날에도 쓰는 말**을 골라라 — 요즘 동화나
  교과서에서 다시 만날 만한 말. 옛 맞춤법으로 적힌 말, 사투리, 지금은 거의 안 쓰는 옛말은
  고르지 마라 — 배워도 쓸 데가 없다.
- 사람 이름·땅 이름 같은 고유명사, 한 글자 낱말, 너무 쉬운 말(엄마, 가다)은 고르지 마라.

sentence: 그 낱말이 든 문장을 대목 그대로 옮겨라. 길면 그 낱말이 든 부분만 잘라도 된다. 고쳐 쓰지 마라.

보기:
- meaning: **그 문장에서** 그 낱말이 뜻하는 바. 아이가 알아듣는 쉬운 말로 짧게 (열다섯 자 안팎).
- wrong1, wrong2: 그럴듯하지만 틀린 뜻 둘. 같은 대목을 보고 헷갈릴 만한 것이면 좋다.
  - 우스꽝스럽거나 너무 쉽게 틀린 게 보이는 것은 쓰지 마라.
  - 뜻이 여럿인 낱말이면 다른 뜻을 틀린 보기로 써도 되지만, 그 문장에서도 말이 되는 뜻은 쓰지 마라.
