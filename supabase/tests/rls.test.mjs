...
// ── bump_growth_on_pass (0010) ────────────────────────
// 통과(verification_pass) 시 스트릭·장르 도장을 올리는 트리거.
// 문민재 결정: 같은 날 여러 번 통과해도 스트릭은 매번 늘리고(하루 1회로 캡하지 않는다),
// 책의 장르 태그 전부에 도장 진행도를 올린다.

// S1 의 streaks 는 위 "0007" 구간에서 이미 (current=7, longest=9, last_passed_on=오늘)
// 로 시드돼 있고, 그 뒤 V_OPEN 통과가 트리거를 한 번 거쳤다 — 7 에서 8 로 늘어야 한다.
const afterFirstPass = await db.query(
  `select current_days, longest_days, last_passed_on from streaks where student_id = $1`, [S1]);
check('V_OPEN 통과가 트리거를 거쳐 스트릭을 7→8 로 늘린다',
  Number(afterFirstPass.rows[0]?.current_days) === 8
...
// 같은 날 두 번째 통과 — 하루 1회로 캡하지 않고 그대로 하나 더 늘어난다.
...
check('같은 날 두 번째 통과도 스트릭을 하나 더 늘린다 (하루 1회로 캡하지 않는다)',
  Number(afterSecondPass.rows[0]?.current_days) === 9
...
check('리셋되어도 longest_days 는 이전 최고치를 유지한다',
  Number(afterGap.rows[0]?.longest_days) === 10, JSON.stringify(afterGap.rows[0]));
